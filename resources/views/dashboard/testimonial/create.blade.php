        ...
        <div class="card mb-6">
            ...
            <div class="card-body pt-4">
                <form method="POST" action="{{ route('dashboard.testimonials.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="row p-5">
                        <h3>{{ __('Add New Testimonial') }}</h3>
                        <div class="mb-4 col-md-6">
                            <label for="name" class="form-label">{{ __('Name') }}</label><span
                                class="text-danger">*</span>
                            <input class="form-control @error('name') is-invalid @enderror" type="text" id="name"
                                name="name" required value="{{ old('name') }}" placeholder="{{ __('Enter user name') }}" autofocus />
                            @error('name')
                        ...
                        <div class="mb-4 col-md-6">
                            ...
                            <input class="form-control @error('position') is-invalid @enderror" type="text" id="position"
                                name="position" value="{{ old('position') }}" required placeholder="{{ __('Enter user position') }}" />
                            @error('position')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="mb-4 col-md-6">
                            ...
                            <input class="form-control @error('review_count') is-invalid @enderror" type="number" id="review_count"
                                name="review_count" min="0" max="5" value="{{ old('review_count') }}" required placeholder="{{ __('Enter rating from 0 to 5') }}" />
                            @error('review_count')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="mb-4 col-md-12">
                            ...
                            <textarea class="form-control @error('review') is-invalid @enderror"
                                ...
                                placeholder="{{ __('Enter user review') }}"  cols="20" rows="5">{{ old('review') }}</textarea>
                            @error('review')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
